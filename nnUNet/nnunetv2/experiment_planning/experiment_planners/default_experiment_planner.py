...
class ExperimentPlanner(object):
    ...
    def determine_resampling(self, *args, **kwargs):
        ...
        # Check for NoNormalization to force nearest neighbor resampling
        normalization_schemes, _ = self.determine_normalization_scheme_and_whether_mask_is_used_for_norm()
        if all([i == 'NoNormalization' for i in normalization_schemes]):
            print("ExperimentPlanner: Using NoNormalization for all input channels -> forcing data resampling to nearest neighbor (is_seg=True)!")
            resampling_data_kwargs["is_seg"] = True
            resampling_data_kwargs["order"] = 0
