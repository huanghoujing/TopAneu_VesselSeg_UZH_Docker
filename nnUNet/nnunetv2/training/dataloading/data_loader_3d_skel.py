...
class nnUNetDataLoader3DSkel(nnUNetDataLoaderBase):
    def generate_train_batch(self):
        ...
        for j in range(len(selected_keys)):
            ...
            # (Lung for example). _select_sample decides which case to load, whether to force foreground, and which
            # fg class to center on (overwrite_class). Default behavior is vanilla nnUNet; subclasses may override.
            i, force_fg, overwrite_class = self._select_sample(j, selected_keys)
            loaded_keys.append(i)
        ...
        if self.transforms is not None:
            with torch.no_grad():
                with threadpool_limits(limits=1, user_api=None):
                    ...
                    # skel is never downsampled for deep supervision (skeleton-recall loss is applied
                    # only at full resolution), so it is always a single stacked full-res tensor.
                    skel_all = torch.stack(skels)
                    del segs, images, skels

        return {'data': data_all, 'target': seg_all, 'skel': skel_all, 'keys': loaded_keys}
