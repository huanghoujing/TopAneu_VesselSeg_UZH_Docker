...
class nnUNetTrainer_warmup(nnUNetTrainer):
    """
    Does a warmup of the entire architecture, then does normal training.

    Ported from official nnUNet, adapted to this repo's nnUNetTrainer.__init__ signature (which has
    `unpack_dataset` and several extra CLI kwargs) via *args/**kwargs pass-through.
    """

    # Class-level defaults so these are present even when a subclass bypasses this __init__ via a direct
    # nnUNetTrainer.__init__ call (as the EnvCfg trainers do). warmup_duration_whole_net is
    # env-configurable via 'warmup_epochs' (read at import; the launch script sets env beforehand).
    warmup_duration_whole_net = int(os.environ.get('warmup_epochs', 50))  # lin increase whole network
    training_stage = None  # 'warmup_all', 'train'

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        #### hyperparameters for warmup
        self.warmup_duration_whole_net = int(os.environ.get('warmup_epochs', 50))
        # NOTE: deliberately do NOT set self.num_epochs here (unlike official nnUNet). This repo's
        # nnUNetTrainer exposes --num_epochs on the CLI; hardcoding it would override that. The base
