...
from nnunetv2.training.loss.dice import get_tp_fp_fn_tn, WeightedMemoryEfficientSoftDiceLoss, WeightedSoftSkeletonRecallLoss
...
# import sys
# sys.path.insert(0, '/data/hohuan/Project/cow_roi_binary_seg/utils')
# from ct_io import save_nifti_file, load_nifti_file

class Tr_SkRecall_NoDeepSupMirrorRotScale_WLoss(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.weight_srec = 1 # This is the default value, you can change it if you want
        self.enable_deep_supervision = False
        self.num_iterations_per_epoch = 250  # Debug
        self.num_val_iterations_per_epoch = 25  # Debug
    
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        """
        This function is stupid and certainly one of the weakest spots of this implementation. Not entirely sure how we can fix it.
        """
        initial_patch_size = patch_size = self.configuration_manager.patch_size
        dim = len(patch_size)
        mirror_axes = None
    ...
    def _build_loss(self):
        loss = DC_SkelREC_and_CE_loss(soft_dice_kwargs={'batch_dice': self.configuration_manager.batch_dice, 
                                                        ...
                                                        'weight': torch.FloatTensor([1, 1, 1, 0.8, 0.8, 0.8, 0.8, 2, 2, 2, 1, 1, 3])
                                                        }, 
                                      ...
                                      ce_kwargs={
                                          'weight': torch.FloatTensor([1, 1, 1, 1, 0.8, 0.8, 0.8, 0.8, 2, 2, 2, 1, 1, 3])
                                          },
                                      weight_ce=1, weight_dice=1, weight_srec=self.weight_srec, 
                                      ignore_label=self.label_manager.ignore_label,
                                      dice_class=WeightedMemoryEfficientSoftDiceLoss, srec_class=WeightedSoftSkeletonRecallLoss).to(self.device)
        return loss
    ...
    def get_training_transforms(
        ...
        if do_dummy_2d_data_aug:
            ...
            ignore_axes = None
        # NoRotScale by setting prob = 0
        transforms.append(
            SpatialTransform(
                patch_size_spatial, patch_center_dist_from_border=0, random_crop=False, p_elastic_deform=0,
                p_rotation=0,
                rotation=rotation_for_DA, p_scaling=0, scaling=(0.7, 1.4), p_synchronize_scaling_across_axes=1,
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # save_keys = {'CT2MR_Tonly_topcow_ct_081', 'CT2MR_Tonly_topcow_ct_089', 'MR2CT_Tonly_topcow_mr_143', 'topcow_ct_047', 'topcow_mr_008', 'topcow_mr_042'}
        # for i in range(batch['data'].shape[0]):
        #     if str(batch['keys'][i]) not in save_keys:
        #         continue
        #     exp_name = '150%Crop'
        #     out_d = f"/data/hohuan/Project/cow_roi_binary_seg/tmp/Aug28_visBatch_{exp_name}/{str(batch['keys'][i])}"
        #     maybe_mkdir_p(out_d)
        #     save_nifti_file(data[i][0].cpu().numpy().astype(np.int16), np.eye(4), f'{out_d}/img.nii.gz')
        #     for j in range(len(target)):
        #         save_nifti_file(target[j][i][0].cpu().numpy().astype(np.int16), np.eye(4), f'{out_d}/segGT_{j}.nii.gz')
        #     save_nifti_file(skel[0][i].cpu().numpy().astype(np.int16), np.eye(4), f'{out_d}/skel.nii.gz')
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        # assert len(predicted_segmentation_onehot.shape) == 5, f"len(predicted_segmentation_onehot.shape) = {len(predicted_segmentation_onehot.shape)}"
        # _shape = list(output.shape)
        # _shape[1] = 1
        # roi_mask = torch.zeros(_shape, dtype=bool, device=self.device)
        # for i, k in enumerate(batch['keys']):
        #     roi_file = join(nnUNet_raw, self.plans_manager.dataset_name, "TopCoW_ROIs", str(k)+'.txt')
        ...
        #     roi_mask[i, :, locaion[0]:locaion[0]+size[0], locaion[1]:locaion[1]+size[1], locaion[2]:locaion[2]+size[2]] = True
        # mask = ~roi_mask # negation of ROI mask since "compute_tp_fp_fn_tn" expects an ignore mask
