...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), output_folder_base: str = None, num_epochs: int = 1000, initial_lr: float = 1e-2, num_iterations_per_epoch: int = 250, cls_lr: float = None):
        ...
        # HOUJING: Use nnUNetTrainer.__init__ signature (not self.__init__) to avoid issues with child classes
        self.my_init_kwargs = {}
        ...
        self.cls_lr = cls_lr  # separate LR for segmentation head; None -> use initial_lr for everything
        ...
        self.print_to_log_file(f'{self.num_iterations_per_epoch = }')
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            if os.getenv('PRINT_NETWORK', '0') == '1':
    ...
    def configure_optimizers(self):
        if self.cls_lr is None:
            optimizer = torch.optim.SGD(self.network.parameters(), self.initial_lr, weight_decay=self.weight_decay,
                                        momentum=0.99, nesterov=True)
            lr_scheduler = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
            return optimizer, lr_scheduler

        seg_head_params, other_params = [], []
        for name, p in self.network.named_parameters():
            if 'seg_layers' in name:
                seg_head_params.append(p)
            else:
                other_params.append(p)
        self.print_to_log_file(f'configure_optimizers: {len(seg_head_params)} seg-head params @ cls_lr={self.cls_lr}, '
                               f'{len(other_params)} other params @ initial_lr={self.initial_lr}')
        param_groups = [
            {'params': other_params, 'lr': self.initial_lr},
            {'params': seg_head_params, 'lr': self.cls_lr},
        ]
        optimizer = torch.optim.SGD(param_groups, self.initial_lr, weight_decay=self.weight_decay,
                                    momentum=0.99, nesterov=True)
        lr_scheduler = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
        return optimizer, lr_scheduler
    ...
    def do_split(self, verbose=True):
        """
    ...
    def on_train_epoch_start(self):
        ...
        if self.cls_lr is not None:
            # Only print to the log file; do not push to self.logger. The MetricLogger variant used by some
            # trainers only accepts pre-declared keys, and an always-present empty 'cls_lrs' list would break
            # its epoch inference (min length over all logged lists) on runs where cls_lr is None.
            self.print_to_log_file(
                f"Current seg-head learning rate: {np.round(self.optimizer.param_groups[1]['lr'], decimals=5)}")
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if hasattr(self, 'debug_output_shape') and self.debug_output_shape:
                print("************* output shapes: ")
    ...
    def perform_actual_validation(self, save_probabilities: bool = False, set_ds_during_infer_to: bool = False, predictor_class=nnUNetPredictor) -> None:
        self.set_deep_supervision_enabled(set_ds_during_infer_to)
        self.network.eval()
        ...
        predictor = predictor_class(tile_step_size=0.5, use_gaussian=True, use_mirroring=True,
