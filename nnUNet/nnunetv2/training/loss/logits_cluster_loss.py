...
class LogitsClusterLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # loss = (F.cross_entropy(input=pred, target=target, reduction='none') * weights).mean()

        return loss

class DC_SkelREC_LCluster_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, soft_skelrec_kwargs, ce_kwargs, lcluster_kwargs, weight_ce=1, weight_dice=1, weight_srec=1, weight_lcluster=1, 
        ...
        self.lcluster = LogitsClusterLoss()
