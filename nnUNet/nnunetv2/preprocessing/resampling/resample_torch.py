    ...
    if all([i == j for i, j in zip(new_shape, data.shape[1:])]):
        ...
        with torch.no_grad():
            ...
            if is_seg:
                ...
                # HOUJING: Per channel interpolation to reduce memory usage
                # print("="*60)
                # print(f"= DEVICE USED IN RESAMPLE: {data.device}")
                # print("="*60)
                result = torch.cat([F.interpolate(data[i:i+1][None].float(), new_shape, mode=torch_mode, antialias=False).cpu()[0] for i in range(data.shape[0])], dim=0)
            if input_was_numpy:
...
def resample_torch_fornnunet(
    ...
    if do_separate_z:
        ...
        return resample_torch_simple(data, new_shape, is_seg, num_threads, device, memefficient_seg_resampling, mode=mode)
